...
impl<'info> Deposit<'info> {
    pub fn deposit(&mut self, amount: u64, max_x: u64, max_y: u64) -> Result<()> {
        ...
        let (x_amount, y_amount, lp_to_mint) = self.calculate_deposit_amounts(amount, max_x, max_y)?;
...
//for depositing mint_x in the vault
    transfer_tokens(&self.user_x, &self.mint_x_vault, &x_amount, &self.mint_x, &self.signer, &self.token_program)?;
//for depositing mint_y in the vault
    transfer_tokens(&self.user_y, &self.mint_y_vault, &y_amount, &self.mint_y, &self.signer, &self.token_program)?;

    //mint lp tokens to the user
        self.mint_lp_tokens(lp_to_mint)?;

        msg!("Deposited {} X and {} Y, minted {} LP", x_amount, y_amount, lp_to_mint);
        Ok(())
    }
    fn calculate_deposit_amounts(&self, lp_amount: u64, max_x: u64, max_y: u64) -> Result<(u64, u64, u64)> {
